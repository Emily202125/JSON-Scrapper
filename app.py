...
def trigger_collection(dataset_id: str, profile_url: str) -> str:
    payload = [{"url": profile_url}]  # only 'url' allowed
    params = {"dataset_id": dataset_id, "include_errors": "true"}
    r = requests.post(TRIGGER_URL, headers=HEADERS, params=params, data=json.dumps(payload),
...
def poll_ready(snapshot_id: str, timeout_s: float = TOTAL_WAIT_SECONDS) -> Dict[str, Any]:
    start = time.time()
    while True:
        r = requests.get(f"{PROGRESS_URL}/{snapshot_id}", headers=HEADERS,
                         timeout=(CONNECT_TIMEOUT, READ_TIMEOUT))
        r.raise_for_status()
...
def download_snapshot_json(snapshot_id: str) -> Union[List[Any], Dict[str, Any]]:
    ...
    r = requests.get(f"{SNAPSHOT_URL}/{snapshot_id}", headers=HEADERS, params=params,
                     timeout=(CONNECT_TIMEOUT, READ_TIMEOUT))
    r.raise_for_status()
    return r.json()
...
with st.sidebar:
    ...
    st.caption("API key is preloaded. Override via secrets/env if needed.")

profile_input = st.text_input("Enter LinkedIn handle or profile URL",
...
if st.button("Fetch JSON"):
    if not API_KEY.strip():
        ...
        try:
            ...
            logs.append(f"snapshot_id: {snap_id}")
            ...
            if status in {"done", "ready", "success", "completed"}:
                ...
                st.warning("Snapshot not ready in 5 seconds. Try again shortly.")
